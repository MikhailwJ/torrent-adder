...
export const synologyAdder = function (server, torrentdata, torrentname) {
  ...
  xhr.open(
    ...
    scheme +
      ...
      ':' +
      server.port +
      '/webapi/auth.cgi?api=SYNO.API.Auth&version=2&method=login&account=' +
      server.login +
      '&passwd=' +
      server.password +
  ...
  if (torrentdata.substring(0, 7) == 'magnet:') {
    ...
    console.log(
      'GET: ' +
        ...
        ':' +
        server.port +
        '/webapi/DownloadStation/task.cgi?api=SYNO.DownloadStation.Task&version=2&method=create&_sid=' +
        sid +
        '&uri=' +
