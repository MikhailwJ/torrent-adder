export const ruTorrentAdder = function (server, data, label, dir) {
  ...
  autoLabelling: if (autolabellist !== null && data.substring(0, 7) != 'magnet:' && !server.rutorrentalwaysurl) {
  ...
  autoDirectory: if (autodirlist !== null && data.substring(0, 7) != 'magnet:' && !server.rutorrentalwaysurl) {
  ...
  url += server.host;
  url += ':' + server.port;
  if (server.ruTorrentrelativepath == undefined || server.ruTorrentrelativepath[0] != '/') url += '/'; // first slash
  if (server.ruTorrentrelativepath != undefined) url += server.ruTorrentrelativepath;
  if (
    server.ruTorrentrelativepath != undefined &&
    server.ruTorrentrelativepath.length != 0 &&
    server.ruTorrentrelativepath[server.ruTorrentrelativepath.length - 1] != '/'
  )
    url += '/'; // trailing slash
  url += 'php/addtorrent.php?';
  if (dir != undefined && dir.length > 0) url += 'dir_edit=' + encodeURIComponent(dir) + '&';
  if (label != undefined && label.length > 0) url += 'label=' + encodeURIComponent(label);
  if (server.rutorrentaddpaused) url += '&torrents_start_stopped=1';
  if (server.rutorrentdontaddnamepath) url += '&not_add_path=1';
...
export const rtorrentXmlRpcAdder = async function (server, torrentdata) {
  var methodName;
  var encodedData;
      ...
      if (!/.*fault.*/.exec(text)) {
        ...
        RTA.displayResponse(
          ...
          'Adding the torrent failed:\n' +
            ...
            ': ' +
            members[1].lastChild.firstChild.textContent,
          true,
        );
      }
    })
