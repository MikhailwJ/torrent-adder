export const uTorrentAdder = function (server, torrentdata) {
  const relpath =
    server.utorrentrelativepath == undefined || server.utorrentrelativepath == ''
      ? '/gui/'
      : server.utorrentrelativepath;
  const scheme = server.hostsecure ? 'https://' : 'http://';

  const tokenUrl = scheme + server.host + ':' + server.port + relpath + 'token.html';
  fetch(tokenUrl)
    ...
    .then(response => {
      if (/<div.*?>(.*?)<\/div>/.exec(response)) {
        ...
        let postUrl = scheme + server.host + ':' + server.port + relpath + '?token=' + token;
        let message;
        const fetchOpts = {
          headers: {},
        };
